...
import { Plus, ReceiptText, CheckCircle, Clock, Trash2 } from 'lucide-react';
...
const Receipts = () => {
  ...
  const [formData, setFormData] = useState({ supplier: '', items: [{ product_id: '', quantity: '' }] });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setFormData({ supplier: '', items: [{ product_id: '', quantity: '' }] });
      fetchAll();
    } catch (err) { alert('Failed: ' + err.message); }
  };

  const validateReceipt = async (id) => {
    const wId = prompt('Enter warehouse ID to receive into:');
    if (!wId) return;
    try {
      await api.post(`/receipts/${id}/validate`, { warehouse_id: parseInt(wId) });
      fetchAll();
    } catch (err) { alert('Failed: ' + err.message); }
  };

  const statusBadge = (s) => {
    const colors = { draft: 'bg-slate-100 text-slate-700 border-slate-200', done: 'bg-emerald-50 text-emerald-700 border-emerald-200', waiting: 'bg-amber-50 text-amber-700 border-amber-200' };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage incoming stock from suppliers.</p>
        </div>
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showForm ? 'max-h-[800px] opacity-100 mb-6' : 'max-h-0 opacity-0'}`}>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
          <h2 className="text-lg font-bold text-slate-800 mb-4">New Receipt</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Supplier *</label>
              <input type="text" value={formData.supplier} onChange={e => setFormData({...formData, supplier: e.target.value})} required
                className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-colors" placeholder="Supplier name" />
            </div>
            <div>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {isLoading ? (
                ...
                <tr><td colSpan="6" className="px-6 py-12 text-center text-slate-500">
                  <ReceiptText size={48} className="mx-auto text-slate-300 mb-4" /><p className="text-lg font-medium text-slate-700">No receipts yet</p>
                </td></tr>
              ) : receipts.map(r => (
                <tr key={r.id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-6 py-4 font-medium text-slate-800">REC-{String(r.id).padStart(4, '0')}</td>
                  <td className="px-6 py-4 text-slate-600">{r.supplier}</td>
                  <td className="px-6 py-4 text-slate-600">{(r.items || []).length} items</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${statusBadge(r.status)}`}>
                      {r.status === 'done' ? <CheckCircle size={14} className="mr-1" /> : <Clock size={14} className="mr-1" />}
                      {r.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-500">{r.created_at ? new Date(r.created_at).toLocaleDateString() : '--'}</td>
